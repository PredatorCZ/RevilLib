...
template <> void FByteswapper(MODMaterialX70 &self, bool way) {
  FByteswapper(self.bitField00);
  FByteswapper(self.vshData, way);
  FByteswapper(self.textureIDs);
...
template <> void FByteswapper(MODMaterialX170 &self, bool way) {
  FByteswapper(self.bitField00);
  FByteswapper(self.bitField01);
  FByteswapper(self.textureIDs);
  FByteswapper(self.transparency);
...
template <class Header, class Traits> MODImpl::ptr LoadMODX70(BinReaderRef rd) {
  ...
  rd.ReadContainer(main.paths, header.numTextures);
