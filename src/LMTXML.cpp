...
Reflector::xmlNodePtr FloatFrame::ToXML(pugi::xml_node &node) const {
...
int FloatFrame::FromXML(pugi::xml_node &node) {
  ...
  for (auto &c : nodeText) {
    if ((c >= '0' && c <= '9') || c == '.')
      ...
      value[numItems] = atof(outputText.c_str());
      outputText.clear();
      numItems++;
    }
  }
...
int LMTFloatTrack_internal::ToXML(pugi::xml_node &node, bool standAlone) const {
  ...
  for (int g = 0; g < GetNumGroups(); g++) {
    ...
    int numEvents = GetGroupTrackCount(g);
...
int LMTFloatTrack_internal::FromXML(pugi::xml_node &floatTracksNode) {
  ...
  for (int e = 0; e < numGroups; e++) {
    ...
    SetNumFrames(e, static_cast<int>(rangeNodes.size()));
...
Reflector::xmlNodePtr AnimEvent::ToXML(pugi::xml_node &node) const {
  int numItems = 0;
...
int LMTAnimationEventV1_Internal::ToXML(pugi::xml_node &node,
  ...
  for (int g = 0; g < GetNumGroups(); g++) {
    ...
    int numEvents = GetGroupEventCount(g);
...
int LMTAnimationEventV1_Internal::FromXML(pugi::xml_node &node) {
  ...
  for (int e = 0; e < numGroups; e++) {
    ...
    SetNumEvents(e, static_cast<int>(rangeNodes.size()));
...
int AnimEventFrameV2::FromXML(pugi::xml_node &node) {
  ...
  for (size_t i = 0; i < valueCopy.size() + 1; i++) {
    if (valueCopy[i] == ',' || valueCopy[i] == '\0') {
      ...
      if (dataType == EventFrameV2DataType::Float)
        fdata[currentItem++] = atof(valueCopy.data() + lastItem);
      else
    ...
    if (a) {
      if (settings == ExportSetting_FullXML)
        ...
        std::string linkedName =
            fleInf.GetFileName() + "_m" + std::to_string(curAniID) + ".mtx";
        ...
        std::string linkedName =
            fleInf.GetFileName() + "_m" + std::to_string(curAniID) + ".mti";
        std::string linkedFullName = fleInf.GetPath() + linkedName;
        cAni.append_buffer(linkedName.c_str(), linkedName.size());
  ...
  for (auto &a : animationNodes) {
    ...
    if (nodeBuffer.empty()) {
      ...
      if (errNo == 1)
        ...
        printerror("[LMT] Layout errors in animation: ",
                   << absolutePath.c_str());
